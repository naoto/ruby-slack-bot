...
class ChatGPTTalk < ChatGPT
  def initialize(options:, logger:)
    ...
    set(/^なおぼっと[[:space:]](.*)$/m, 'ChatGPTと会話する') { |data:, matcher:| message_talk(data, matcher) }
    set(/^job$/, 'ChatGPTのシステムプロンプトを表示する') { |data:, matcher:| job_display(data, matcher) }
    set(/^job[[:space:]](.*)$/m, 'ChatGPTのシステムプロンプトを設定する') { |data:, matcher:| job_set(data, matcher) }
    set(/^job[[:space:]]reset$/, 'ChatGPTのシステムプロンプトをリセットする') { |data:, matcher:| job_reset(data, matcher) }
    set(/^talk[[:space:]]reset$/, 'ChatGPTの会話履歴をリセットする') { |data:, matcher:| talk_reset(data, matcher) }
  end
